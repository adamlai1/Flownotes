...
export function posKey(projectId, contextId, itemId) {
...
export function lloydSpread(items, width, height, headerH, bottomPad, pinnedNoteIds = null) {
...
export function clusterEllipse(bubs) {
...
export function arrangeNotesAroundBubbles(items, anchoredIds, width, height, safeBottom = 0) {
  ...
  for (let iter = 0; iter < 120; iter++) {
    ...
    for (const p of pos) {
      ...
      if (pen > 0) {
        ...
        score += pen
        // Corner pocket. The radial push points down-right, where the page bounds
        // are, so for an item wedged between the button and the corner the bounds
        // clamp below cancels the push exactly and the item never leaves the zone —
        // `moved` stays true, the iteration cap burns out, and the pass hands back a
        // best-effort snapshot. On the ADD path this is the common case: a freshly
        // created note starts on a fresh-layout slot that a saved-position item
        // already occupies, gets shoved outward, and the pocket is the sink it lands
        // in. Worse, the post-loop ejection then lifted every trapped item to the same
        // point with no iterations left to pull them apart — two notes stacked
        // exactly, which is the 35–46 px "overlap on arrival" the harness measures.
        // So eject HERE, inside the loop, the moment the clamp would cancel the push:
        // the same guaranteed up/left axis move the drag clamp uses, and the pair
        // separation on the next iteration resolves whatever it lands on. Pure
        // control flow — no gap, damping, or iteration value changes.
        const cx0 = p.cx, cy0 = p.cy
        boundsClamp(p)
        if ((Math.abs(p.cx - cx0) > EPS || Math.abs(p.cy - cy0) > EPS) &&
            circleBoxPen({ cx: btnCx, cy: btnCy, r: PLUS_BTN_EXCL_R }, p, BTN_ROW_PAD - EPS) > 0) {
...
export function settleItems(items, anchoredIds, width, height, safeBottom = 0) {
  const GAP = 16
...
export default function BubbleVisualization({
  ...
  // level's saved positions; it overwrites the arrangement with no undo.
  ...
  return (
    ...
    >
      ...
      >
        ...
        <>
        {/* Breadcrumb — back arrow + trail sit tight to the left edge. Wraps to
            two lines; past that levels drop one at a time by keep-priority
          ...
          {crumbCollapsed.length > 0 && (
            <span className="relative flex items-center gap-0.5 flex-shrink-0">
              ...
              {crumbMenuOpen && (
                ...
                >
                  {crumbCollapsed.map(c => (
                    <button
                      key={c.id}
                      onClick={() => { setCrumbMenuOpen(false); zoomOutTo(c.depth) }}
                      className="block w-full text-left px-4 py-2.5 text-sm truncate transition-colors"
        ...
        <div className="relative">
          ...
          {layoutMenuOpen && (
            ...
            >
              ...
              {/* "Default" is a one-shot action, not a mode: it overwrites this
                  level's saved positions with the physics spawn layout and leaves
                  the level in Free. Hidden while Sorted (saved positions aren't in
                  play there), like the old canvas Reorganize button; styled as an
                  action row — icon, muted, never the active highlight — so it
                  doesn't read as a fourth view. Disabled when there's nothing to
                  reset. */}
              {!isSorted && (
                <button
                  onClick={() => { setLayoutMenuOpen(false); setConfirmReorganize(true) }}
                  disabled={!hasManualLayout || layoutItems.length <= 1}
                  className="w-full text-left px-4 py-2 transition-colors flex items-center gap-2.5 disabled:opacity-40"
                  style={{ color: 'var(--text-2)' }}
                >
                  <svg className="w-3.5 h-3.5 flex-shrink-0 opacity-70" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    <rect x="4" y="4" width="7" height="7" rx="1.5" strokeWidth={2} />
                    <rect x="13" y="4" width="7" height="7" rx="1.5" strokeWidth={2} />
                    <rect x="4" y="13" width="7" height="7" rx="1.5" strokeWidth={2} />
                    <rect x="13" y="13" width="7" height="7" rx="1.5" strokeWidth={2} />
                  ...
                  <span>
                    <span className="block text-sm leading-tight">Default</span>
                    <span className="block text-[11px] leading-tight" style={{ color: 'var(--text-2)', opacity: 0.6 }}>
                      Resets bubble positions
                    </span>
                  </span>
                </button>
              )}
      ...
      <ConfirmDialog
        ...
        message="This replaces your arrangement on this level with the automatic layout. This can't be undone."
        confirmLabel="Reset"
